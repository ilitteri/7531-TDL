...
fn read_forms(
    ...
    mutex: &Arc<Mutex<Vec<ClientAccount>>>,
    dni_user: &mut String,
) -> Result<u8, u8> {
    ...
    *dni_user = aux_dni;
    ...
    _index += 1 as usize;
    _password = Some(bytes2string(
        &buffer_packet[_index..(_index + password_size)],
    )?);
    ...
    _index += 1 as usize;
    _lastname = Some(bytes2string(
        &buffer_packet[_index..(_index + lastname_size)],
    )?);
    ...
    _index += 1 as usize;
    _birth_date = Some(bytes2string(
        &buffer_packet[_index..(_index + birth_date_size)],
    )?);
    ...
    _index += 1 as usize;
    _priority = Some(bytes2string(
        &buffer_packet[_index..(_index + priority_size)],
    )?);
...
fn leer_contenido_log(
    buffer_packet: Vec<u8>,
    lock: &Arc<Mutex<Vec<ClientAccount>>>,
    algo: &mut String,
...
pub fn delete_user(dni_user: &mut String, mutex: &Arc<Mutex<Vec<ClientAccount>>>) {
    ...
    match vector
        .iter()
        .position(|x| x.get_dni() == Some(dni_user.clone()))
    {
        Some(x) => {
            vector.remove(x);
        }
