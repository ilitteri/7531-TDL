...
pub fn write_json(path: &str, form: ClientAccount) -> Result<(), serde_json::Error> {
    ...
    if json_file_path.exists() {
        let _file = match File::open(&json_file_path) {
            Err(why) => panic!(
                "La información del cliente no pudo ser guardada. Motivo: couldn't open {}: {}",
                display, why
            ),
            Ok(_file) => _file,
        };
        ...
        let _file = match File::create(&json_file_path) {
            Err(why) => panic!(
                "La informacion del cliente no pudo ser guardada. Motivo: couldn't create {}: {}",
                display, why
            ),
            Ok(_file) => _file,
        };
...
pub fn find_user(
    ...
    dni: String,
) -> Result<ClientAccount, ReadError> {
    let vector = mutex.lock().unwrap();
